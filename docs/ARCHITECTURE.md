...
Абракция отвечающая за представление аргументов комманд. Когда исходный ввод пользователя парсится и происходит создание обхектов комманд, тогда же конструируются и аргументы комманд, после чего сохраняются как их поля (в поле `args`, в случае если комманды их предусматривает).

Глобально тут есть два имплементатора этого класса:
- `DoubleQuotesArgument`: хранит строковое представление аргумента as-is, то есть не подставляет переменные окружения и экранирует спец. символы по типу `\n` и `\t`.
  - Экранизация спец. символов происходит сразу, как `DoubleQuotesArgument` был сконсруирован и кешируется в поле `str`.
- `SmartArgument`: этот тип аргументов применяет подстановку переменных из окружения и не экранирует спец. символы (его наследники это `SingleQuotesArgument` и `NoQuotesArgument`).
  - Подстановка переменных окружения происходит при попытке получить представление аргумента как строчки (в момент непосредственного выполнения комманды). Конкретно подстановка происходит при вызове метода `Argument::evaluateAsString(Environment)`, в который передается окружение. Из него будут браться значения для переменных окружения, и если какая-та переменная там отсутствует, то вместо нее подставится пустая строка.
