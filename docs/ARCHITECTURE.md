...
Все кастомные комманды (`cat`, `echo`, `wc`, `grep`, `pwd`, `cd`, `ls`, `exit`) и те, что требуют запуска отдельного процесса, являются наследниками интерфейса `Command` и получают нужный им для работы набор аргументов в контрукторе.
